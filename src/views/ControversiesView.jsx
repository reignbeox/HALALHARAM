            ...
            <div key={idx} className={`border-2 border-zinc-700 bg-[#050505] transition-all duration-300 ${activeControversy === idx ? 'shadow-[8px_8px_0_#ccff00] border-[#ccff00]' : 'hover:border-zinc-500'}`}>
              <button onClick={() => setActiveControversy(activeControversy === idx ? null : idx)} className="w-full bg-[#111111] p-5 border-b-2 border-zinc-700 flex items-center justify-between group">
                <div className="flex items-center gap-4 text-left">
                  ...
                  <h3 className="text-xl md:text-2xl font-black tracking-tighter uppercase text-white">{item.topic}</h3>
                </div>
                <div className={`p-1 bg-[#050505] border transition-colors flex-shrink-0 ${activeControversy === idx ? 'border-[#ccff00] text-[#ccff00]' : 'border-zinc-700 text-zinc-500 group-hover:border-zinc-500'}`}>
              ...
              {/* Increased max-h to 2000px so it doesn't clip your new data */}
              <div className={`overflow-hidden transition-all duration-500 ease-in-out ${activeControversy === idx ? 'max-h-[2000px] border-t-2 border-zinc-800 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="p-6 md:p-8 bg-[#050505]">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {item.sources.map((s, i) => (
                        <button 
                          ...
                          className={`inline-flex items-center gap-3 px-4 py-3 bg-zinc-900 border border-zinc-700 text-left group transition-colors ${s.link ? 'hover:border-[#ccff00] cursor-pointer' : 'cursor-default'}`}
                        >
                          <ExternalLink className={`w-4 h-4 transition-colors ${s.link ? 'text-zinc-500 group-hover:text-[#ccff00]' : 'text-zinc-600'}`} />
                          <div>
                            <span className={`block font-bold text-xs uppercase tracking-wider transition-colors ${s.link ? 'text-white group-hover:text-[#ccff00]' : 'text-zinc-400'}`}>{s.title}</span>
                            <span className="block text-[10px] font-mono text-zinc-500 uppercase tracking-widest">AUTHOR: {s.author}</span>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {/* NEW: Contemporary Sheikhs Section */}
                    ...
                    <div className="mt-8 pt-8 border-t border-zinc-800">
                      ...
                      <div className="space-y-4">
                        {item.contemporaryScholars.map((scholar, sIdx) => (
                          <div key={sIdx} className="bg-[#111111] border border-zinc-800 p-5 hover:border-zinc-600 transition-colors">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3 border-b border-zinc-800 pb-3">
                              <span className="font-black text-white uppercase tracking-wider">{scholar.name}</span>
                              {scholar.source && (
                                <button 
                                  onClick={() => scholar.source.link && window.open(scholar.source.link, '_blank', 'noopener,noreferrer')}
                                  className={`inline-flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-zinc-700 text-[9px] font-mono uppercase tracking-widest transition-colors ${scholar.source.link ? 'text-zinc-400 hover:border-[#ccff00] hover:text-[#ccff00] cursor-pointer' : 'text-zinc-600 cursor-default'}`}
                                >
