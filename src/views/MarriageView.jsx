...
export default function MarriageView() {
    ...
    // Helper to render the sliding detail panel to avoid code duplication
    const renderDetailPanel = (category, data) => (
      <div className={`detail-panel absolute inset-x-0 bottom-0 top-[60px] bg-[#050505] z-30 p-6 flex flex-col transition-transform duration-300 ease-in-out ${activeItem?.category === category ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        {activeItem?.category === category && (
          <div className="flex-grow overflow-y-auto">
            <h4 className="detail-title font-black text-2xl text-[#ccff00] uppercase tracking-tighter mb-4 leading-none">{data[activeItem.index].title}</h4>
            <p className="detail-text font-mono text-white text-sm leading-relaxed mb-6">{data[activeItem.index].elaboration}</p>
            <div className="border-t border-zinc-800 pt-4 marriage-divider">
              <span className="block font-mono text-[9px] text-zinc-500 uppercase tracking-widest mb-3 font-bold">SOURCES:</span>
              {data[activeItem.index].sources.map((s, i) => (
                <div key={i} className="source-item bg-zinc-900 border border-zinc-800 p-3 mb-2 flex items-center gap-3">
                  <ExternalLink className="w-3 h-3 text-[#ccff00] flex-shrink-0" />
                  <div>
                    <div className="source-title font-mono font-bold text-white text-[10px] uppercase tracking-wider">{s.title}</div>
    ...
    return (
      <section className="px-6 py-16 relative z-10 bg-transparent flex-grow">
        <div className="max-w-7xl mx-auto">
          <div className="mb-12">
            <h2 className="section-title text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-4">MARRIAGE: RIGHTS & RESPONSIBILITIES</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* HUSBAND COLUMN */}
            <div className="marriage-card border-2 border-[#ccff00] bg-[#050505] flex flex-col overflow-hidden relative min-h-[400px]">
              ...
              <div className={`p-6 flex flex-col gap-6 flex-grow relative z-10 transition-opacity duration-300 ${activeItem?.category === 'husband' ? 'opacity-0' : 'opacity-100'}`}>
                {MARRIAGE_DATA.husband.map((item, idx) => (
                  <button key={idx} onClick={() => setActiveItem({ category: 'husband', index: idx })} className="relative text-left group">
                    ...
                    <h4 className="marriage-item-title font-mono font-bold text-[#ccff00] group-hover:text-white transition-colors text-sm uppercase tracking-widest mb-2 pl-2">{item.title}</h4>
              ...
              {renderDetailPanel('husband', MARRIAGE_DATA.husband)}
            </div>
  
            {/* SHARED COLUMN - Kept the original size/overlap logic */}
            <div className="marriage-card border-2 border-[#ccff00] bg-[#050505] flex flex-col lg:-mt-4 lg:-mb-4 z-20 overflow-hidden relative min-h-[400px]">
              <div className="bg-[#ccff00] text-black p-4 border-b-2 border-[#ccff00] flex items-center gap-3 relative z-20">
                <Scale className="w-5 h-5" />
                <h3 className="font-black text-xl tracking-tighter uppercase">SHARED RIGHTS</h3>
              </div>
              <div className={`p-6 flex flex-col gap-6 flex-grow justify-start relative z-10 transition-opacity duration-300 ${activeItem?.category === 'shared' ? 'opacity-0' : 'opacity-100'}`}>
                {MARRIAGE_DATA.shared.map((item, idx) => (
                  <button key={idx} onClick={() => setActiveItem({ category: 'shared', index: idx })} className="text-left group w-full flex flex-col items-start">
                    <h4 className="marriage-item-title font-mono font-bold text-[#ccff00] group-hover:text-white transition-colors text-lg uppercase tracking-widest mb-2">{item.title}</h4>
              ...
              {renderDetailPanel('shared', MARRIAGE_DATA.shared)}
            </div>
            ...
            <div className="marriage-card border-2 border-[#ccff00] bg-[#050505] flex flex-col overflow-hidden relative min-h-[400px]">
              ...
              <div className={`p-6 flex flex-col gap-6 flex-grow relative z-10 transition-opacity duration-300 ${activeItem?.category === 'wife' ? 'opacity-0' : 'opacity-100'}`}>
              ...
              {renderDetailPanel('wife', MARRIAGE_DATA.wife)}
            </div>
