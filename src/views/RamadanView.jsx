      ...
      <section className="px-6 py-16 relative z-10 bg-[#0a0a0a] flex-grow">
        <div className="max-w-7xl mx-auto">
          {!activeItem ? (
            <>
              <div className="mb-12">
                <div className="inline-block px-4 py-1 border border-[#ccff00] text-[#ccff00] font-mono text-[10px] uppercase tracking-[0.3em] mb-4 font-bold bg-[#ccff00]/10">SEASONAL UPLINK ACTIVE</div>
                <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-4">THE RAMADAN GUIDE</h2>
                <p className="font-mono text-zinc-400 uppercase tracking-widest text-sm border-l-4 border-[#ccff00] pl-4">Identify exact parameters of fast invalidation based on classical Fiqh.</p>
              </div>
              ...
              {sortedData.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {sortedData.map(item => (
                    <button key={item.id} onClick={() => navigateTo('ramadan', item.topic)} className="w-full text-left p-6 bg-[#050505] border-t-4 border-zinc-800 hover:border-[#ccff00] group transition-all flex flex-col justify-between">
                      <div className="mb-6">
                        <div className="flex items-center gap-2 font-mono text-[10px] mb-3 uppercase tracking-widest text-zinc-500">
                          <span>{getStatusDot(item.status)}</span><span className="font-bold">{item.status}</span>
                        </div>
                        <h4 className="font-black text-white group-hover:text-[#ccff00] transition-colors uppercase tracking-tight mb-3 text-xl leading-tight">{item.topic}</h4>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="max-w-4xl mx-auto space-y-8">
              <button onClick={() => navigateTo('ramadan')} className="mb-8 flex items-center gap-3 px-4 py-2 bg-zinc-900 border border-zinc-700 text-zinc-400 hover:text-black hover:bg-[#ccff00] hover:border-[#ccff00] font-mono text-[10px] font-bold uppercase tracking-[0.2em] transition-all w-fit">
