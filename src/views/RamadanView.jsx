...
import { RAMADAN_DATABASE } from '../data/database';
import { getStatusConfig, getStatusDot, getTabDot } from '../utils/helpers';

export default function RamadanView({ activeTopic, navigateTo }) {
    const [ramadanFilter, setRamadanFilter] = useState("ALL");
    const [expandedDiffering, setExpandedDiffering] = useState({});
  
    const filteredData = RAMADAN_DATABASE.filter(item => {
      if (ramadanFilter === "ALL") return true;
      if (ramadanFilter === "BREAKS FAST" && item.status === "BREAKS FAST") return true;
      if (ramadanFilter === "DOES NOT BREAK" && item.status === "DOES NOT BREAK") return true;
      if (ramadanFilter === "DISPUTED / MAKRUH" && (item.status === "DISPUTED" || item.status === "MAKRUH")) return true;
      return false;
    });
  
    const getRamadanStatusWeight = (status) => {
      const s = status.toUpperCase();
      if (s === 'DOES NOT BREAK') return 1;
      if (s === 'DISPUTED' || s === 'MAKRUH') return 2;
      if (s === 'BREAKS FAST') return 3;
      return 4;
    };
  
    const sortedData = [...filteredData].sort((a, b) => {
      if (ramadanFilter === "ALL") {
        const weightDiff = getRamadanStatusWeight(a.status) - getRamadanStatusWeight(b.status);
        if (weightDiff !== 0) return weightDiff;
      }
      return a.topic.localeCompare(b.topic);
    });
  
    const activeItem = activeTopic ? RAMADAN_DATABASE.find(i => i.topic === activeTopic) : null;
      ...
      <section className="px-6 py-16 relative z-10 bg-transparent flex-grow">
        <div className="max-w-7xl mx-auto">
          {!activeItem ? (
            <>
              <div className="mb-12">
                <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-4">THE RAMADAN GUIDE</h2>
              ...
              {sortedData.length === 0 ? (
                ...
                {sortedData.map(item => (
                  <button 
                    key={item.id} 
                    onClick={() => navigateTo('ramadan', item.topic)} 
                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-zinc-500 bg-transparent">
            ...
            <div className="max-w-4xl mx-auto space-y-8">
              <button onClick={() => navigateTo('ramadan')} className="mb-8 flex items-center gap-3 px-4 py-2 bg-zinc-900 border border-zinc-700 text-zinc-500 hover:text-black hover:bg-[#ccff00] hover:border-[#ccff00] font-mono text-[10px] font-bold uppercase tracking-[0.2em] transition-all w-fit">
                <span className="text-lg leading-none mb-0.5">&lt;</span><span>RETURN TO RAMADAN GUIDE</span>
              </button>
              ...
              <div className="bg-[#0a0a0a] border-2 border-zinc-800 relative group transition-colors hover:border-zinc-700">
                ...
                <div className="p-8 md:p-12">
                  ...
                  <div className="mb-12 relative">
                    ...
                    {/* text-white allows it to flip to black in light mode */}
                    <p className="text-white leading-relaxed text-lg font-mono">{activeItem.coreReasoning}</p>
                  </div>
                      ...
                      <tbody className="divide-y divide-zinc-800 bg-[#050505]">
                        {activeItem.schoolsOfThought.map((school, idx) => (
                          <tr key={idx} className="hover:bg-zinc-900/50 transition-colors">
                            ...
                            {/* text-white for flipping */}
                            <td className="py-5 px-6 text-sm font-mono text-white">{school.ruling}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="pt-8 border-t-2 border-zinc-800">
                    ...
                    <div className="flex flex-col sm:flex-row flex-wrap gap-4 mb-8">
                      {activeItem.sources.map((source, idx) => (
                        ...
                        >
                          <div className="bg-black p-2 border border-zinc-700 group-hover:border-[#ccff00] transition-colors"><ExternalLink className="w-4 h-4 text-zinc-500 group-hover:text-[#ccff00]" /></div>
                          <div>
                    ...
                    {activeItem.differingOpinions && (
                      <div className="mt-6 border-t border-zinc-800 pt-6">
                        <button onClick={() => setExpandedDiffering(p => ({ ...p, [activeItem.id]: !p[activeItem.id] }))} className="flex items-center gap-3 text-xs font-mono font-bold text-zinc-500 hover:text-white transition-colors uppercase tracking-widest">
